...
function startNewGame(){
  ...
  if (!home || !away){ logLine("Pick BOTH a Home team and an Away team."); return; }
  if ((home.players||[]).length < 2 || (away.players||[]).length < 2){
    logLine("Each team needs at least 2 players (and ideally 1 pitcher).");
    return;
  }
...
function bindEvents(){
  ...
  $("#homeTeamSelect").addEventListener("change", (e)=> app.homeTeamId = e.target.value);
  $("#awayTeamSelect").addEventListener("change", (e)=> app.awayTeamId = e.target.value);
